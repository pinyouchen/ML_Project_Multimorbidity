...
class Visualizer:
    ...
    def plot_shap_summary_oof(self, shap_values, X_test):
            ...
            try:
                ...
                fig = plt.gcf()
                fig.set_size_inches(10, 12) #在這裡調整圖片大小
    ...
    def plot_pr_curve_with_ci(self, precisions, mean_recall, pr_aucs, no_skill):
        fig, ax = plt.subplots(figsize=(8, 6))
        # 注意: precisions 是一個 list of arrays，長度可能不同，需要插值處理
        # 這裡為了簡化，假設外部已經做過插值，傳入的是統一長度的矩陣
        # 如果外部傳入的是 list of varying length arrays，這裡會報錯
        # 因此我們加上一個簡單的處理或假設外部已處理
    ...
    def plot_correlation_matrix(self, df, method='pearson'):
        ...
        sns.heatmap(
            ...
            square=True, linewidths=.5, cbar=True, # cbar_kws removed to simplify
